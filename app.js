document.addEventListener('DOMContentLoaded', () => {
    ...
    const layerOSM = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap' });
    const layerSat = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { attribution: 'Tiles © Esri' });
    ...
    const markerClusterGroup = L.markerClusterGroup({ disableClusteringAtZoom: 16, maxClusterRadius: 50 });
    ...
    map.on('locationfound', function(e) {
        ...
        if (!locationMarker) {
            map.eachLayer((layer) => { if(layer.options && layer.options.color === '#3b82f6') map.removeLayer(layer); });
    ...
    function jumpToSearchResult(lat, lon, name) {
        ...
        toggleMenu(false); 
        if(autoFollow) { autoFollow = false; updateLocationButtonUI(); map.stopLocate(); }
    ...
    document.getElementById('btn-report').addEventListener('click', async () => {
        ...
        try {
            ...
            toggleMenu(false);
        } catch(e) { customAlert(t('alertError')); }
    });

    const filterSelectors = ['filter-public', 'filter-eurokey', 'filter-changing', 'filter-open', 'filter-success', 'filter-favorites'];
    filterSelectors.forEach(id => document.getElementById(id).addEventListener('change', renderMarkers));
    ...
    function renderMarkers() {
        ...
        allToilets.forEach(toilet => {
            ...
            const isExplicitEurokey = tags['centralkey'] === 'eurokey' || tags['eurokey'] === 'yes' || tags.access === 'centralkey' || tags['toilets:eurokey'] === 'yes' || tags['toilets:centralkey'] === 'eurokey';
            ...
            let baseClass = isEurokeyOrWheelchair ? 'text-2xl bg-yellow-300 dark:bg-yellow-600 rounded-full border-2 border-yellow-500 p-1 shadow-md relative' : 'text-2xl relative bg-white dark:bg-gray-600 rounded-full border border-gray-200 dark:border-gray-500 p-1 shadow-md';
            ...
            // HIER DIE ÄNDERUNG (isTopRated Parameter):
            marker.on('click', () => { 
    ...
    // HIER DIE ÄNDERUNG (isTopRated in Funktionssignatur):
    async function openSheet(toilet, isEurokeyOrWheelchair, isExplicitEurokey, isWheelchair, is247, hasChanging, isDefect, isTopRated, lat, lon) {
        ...
        if (userLocation) {
            ...
            }).catch(e => console.log("Routing fallback active")); 
        }
        ...
        } else distEl.innerText = '';

        const subtitleEl = document.getElementById('sheet-subtitle');
        if (tags.name) { subtitleEl.innerText = tags.name; subtitleEl.classList.remove('hidden'); }
        ...
        if (addressParts.length > 0) {
            ...
            if (addressCache[cacheKey] !== undefined) {
                ...
                .catch(() => addressEl.classList.add('hidden'));
            }
        }
        ...
        if (tags.level !== undefined) {
            ...
            else if (lvl < 0) lvlTxt += t('levelUG', {lvl: lvl});
        ...
        let genderInfo = '';
        if (tags.unisex === 'yes') {
            genderInfo = 'Zugang: Unisex / Alle Geschlechter';
        } else if (tags.male === 'yes' && tags.female === 'yes') {
            genderInfo = 'Zugang: Damen & Herren';
        } else if (tags.female === 'yes') {
            genderInfo = 'Zugang: Nur Damen';
        } else if (tags.male === 'yes') {
            genderInfo = 'Zugang: Nur Herren';
        }

        // HIER DIE ÄNDERUNG (isTopRated Tag):
        let info = [];
        ...
        document.getElementById('btn-navigate').onclick = () => {
            ...
            if (isIOS) window.location.href = `http://maps.apple.com/?daddr=${lat},${lon}`;
            else window.location.href = `https://www.google.com/maps/dir/?api=1&destination=${lat},${lon}`;
        };
    ...
    async function loadRatings(osmId) {
        try {
            ...
            if (totalVotes === 0) document.getElementById('stat-usable').innerText = t('statNoData');
            else {
            ...
            if (cleanCount === 0) document.getElementById('stat-clean').innerText = t('statNoRating');
            else {
    ...
    async function sendVote(payload) {
        ...
        try {
            await fetch('backend.php', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) });
